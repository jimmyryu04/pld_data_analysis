...
def extract_sequence(fasta_output):
    """Extract sequence from FASTA-formatted pldesign output (2nd line only)."""
...
def run_pldesign(params):
    ...
    return extract_sequence(result.stdout)
...
#     """Find motif match region in UTR and return masking region strings."""
...
#             start = utr_upper.find(q_upper)
#             if start != -1:
#                 full_len = len(prot_seq) * 3 + len(utr_seq)
#                 start += len(prot_seq) * 3
#                 end = start + len(qseq) - 1
#             # 여기에 start -5 , -3 / end +3, +5 등등의 조건 바꿔가면서 masking하는 부위 수정. -> 현재는 정말 motif에 딱 맞춰서 region 잡음.    

#                 return (
#                     f"{start},{end},1,{start}",
#                     f"{start},{end},{end},{full_len}",
#                     f"1,{start},{start},{end}",
#                     f"{end},{full_len},{start},{end}"
#                 )
#     return None
...
def run_case(args):
    ...
    case_type, utr_id1, utr_id2, utr_seq, lam, prot_id, prot_seq, result_list, progress, task_id, tmp_path = args
    if case_type == 'cds':
        case_name = f"{prot_id}_{utr_id1}_{utr_id2}_lam{lam}_CTRL"
        cds_seq = run_pldesign(["-p", prot_seq, "-l", str(lam), "--fasta", "--exclude-start", "5"])
    elif case_type == 'utr':
        case_name = f"{prot_id}_{utr_id1}_{utr_id2}_lam{lam}_CFLD"
        full_seq = run_pldesign(["-p", prot_seq, "-l", str(lam), "-u", utr_seq, "--fasta", "--exclude-start", "5"])
        cds_seq = full_seq[:len(full_seq) - len(utr_seq)]
    elif case_type == 'mask':
        case_name = f"{prot_id}_{utr_id1}_{utr_id2}_lam{lam}_MASK"
        mask_regions = get_mask_region(prot_seq, utr_id1, utr_seq)
        if mask_regions:
            cmd = ["-p", prot_seq, "-l", str(lam), "-u", utr_seq]
            for region in mask_regions:
                cmd += ["-m", region]
            cmd += ["--fasta", "--exclude-start", "5"]
            full_seq = run_pldesign(cmd)
            cds_seq = full_seq[:len(full_seq) - len(utr_seq)]
        else:
            return
    else:
    ...
    case_name, mfe, cai, full_seq, struct = process_case(case_name, cds_seq, utr_seq)
    result_list.append((prot_id, case_type, case_name, mfe, cai, full_seq, struct))

    with open(tmp_path, "a") as tf:
        tf.write(f"{prot_id}\t{case_type}\t{case_name}\t{len(full_seq)}\t{mfe:.2f}\t{cai:.3f}\t{full_seq}\t{struct}\n")
...
@click.option('--process', '-p', type=int, default=4, help='Number of processes to use.')
def main(utr3_fwd, utr3_bwd, cds_fasta, output_dir, output_prefix,
         cds_only, with_utr, with_mask, lambda_values, process):
    ...
    with Progress(
        ...
        for cds_record in cds_records:
            ...
            for i in range(len(task_args)):
                ...
                task_args[i][7] = results  # overwrite result_list reference
