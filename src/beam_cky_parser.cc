...
double BeamCKYParser<ScoreType, IndexType, NodeType>::get_broken_codon_score(
    ...
    if (t_index / 3 < (IndexType)protein.size()){
...
void BeamCKYParser<ScoreType, IndexType, NodeType>::hairpin_beam(IndexType j, DFA_t& dfa) {
    ...
    //    2. generate p(i, j)
    for (IndexType i_node_nucpair_ = 0; i_node_nucpair_ < 16 * j; ++i_node_nucpair_) {
...
void BeamCKYParser<ScoreType, IndexType, NodeType>::P_beam(IndexType j, DFA_t& dfa){
    ...
    auto j_node = make_pair(j, j_num);
    struct BasepairingMask masks[max(1, pld->num_basepairing_masks)];
    int num_masks = 0;

    for (int k = 0; k < pld->num_basepairing_masks; k++) {
        if (pld->basepairing_masks[k].start3 <= j &&
                j < pld->basepairing_masks[k].end3) {
            masks[num_masks] = pld->basepairing_masks[k];
            num_masks++;
        }
    }
                ...
                for (auto &i_1_node_nuci_1 : dfa.left_edges[i_node]){
                    ...
                    if (_allowed_pairs[nuci_1][nucj]){
                        ...
                        for (int k = 0; k < num_masks; k++)
                            if (masks[k].start5 <= i && i < masks[k].end5) {
                                cai_score = -32768.0;
                                break;
                            }
    ...
    // M2 = M + M_P
    for (IndexType i_node_ = 0; i_node_ < 2 * j; ++i_node_) {
        ...
        if (i > 0 and j < seq_length){
            for (IndexType m_node = 0; m_node < 2 * i; ++m_node){
...
template <typename ScoreType, typename IndexType, typename NodeType>
BeamCKYParser<ScoreType, IndexType, NodeType>::BeamCKYParser(
    const double lambda_value, const bool verbose, struct PLDOptions *pld_options)
