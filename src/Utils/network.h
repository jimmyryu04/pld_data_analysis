...
DFAType get_dfa(string &protein_seq, string& utr_seq) {
    DFAType dfa = DFAType();
    string line;
    ...
    while (getline(std::cin, line))
        ...
        case 'R':
            ...
            fields = util::split(line, ' ');
            weight = stod(fields[2]);
            num2 = stoi(fields[3]);
            int nuc = GET_ACGU_NUC(fields[1][0]);
            auto next_node = make_pair(current_node.first + 1, num2);

            dfa.add_edge(current_node, next_node, nuc, weight);
            break;
        }
#ifdef debug_dfa
    printf("--- BEGIN OF DFA DUMP ---\n");
    for(IndexType pos = 0; pos < 3 * static_cast<IndexType>(protein_seq.size()) + 1; pos++){
        if (pos > 0)
            printf("\n");
