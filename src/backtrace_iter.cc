...
BacktraceResult BeamCKYParser<ScoreType, IndexType, NodeType>::backtrace(DFA_t& dfa, const State_t& state, NodeType end_node){
    ...
    while(!stk.empty()) {
        ...
        IndexType i = 0, j, /*p,*/ q, hairpin_length;
        j = j_node.first;
        ...
        switch (beam_type) {
            case Beam_type::BEAM_C:
                ...
                // C = C + P
                if(no_backpointer) {
                    for (IndexType c_node_nucpair_ = 0; c_node_nucpair_ < 16 * seq_length; ++c_node_nucpair_){
                    ...
                    if (hairpin_length == 5 or hairpin_length == 6 or hairpin_length == 8){
                        for(auto & seq_score_weight : hairpin_seq_score_cai[i_node][j_1_node][NTP(nuci, nucj_1)]){
                            ...
                            if (state.score == pre_cal_score && abs(state.cai_score - pre_cal_cai_score) < epsilon){
                                for(int c=0; c<(IndexType)seq.size(); c++)
                                    sequence[i+c] = seq[c];
                    ...
                    for (auto& i1_node_nuci : dfa.right_edges[i_node]){
                        ...
                        for (auto& j_2_node_nucj_2 : dfa.left_edges[j_1_node]){
                            ...
                            for (auto& i2_node_nuci1 : dfa.right_edges[i1_node]){
                                ...
                                auto f14_value = func14(i, j-1, i+1, j-2, nuci, nuci1, nucj_2, nucj_1,
                                                                    nuci, nuci1, nucj_2, nucj_1);
                                ScoreType newscore;
                                if (p_state.score == util::value_min<ScoreType>())
                                    newscore = util::value_min<ScoreType>();
                                else if (f14_value == util::value_max<ScoreType>())
                                    newscore = util::value_min<ScoreType>();
                                else
                                    newscore = - f14_value + p_state.score;

                                auto cai_score = p_state.cai_score + (weight_nuci + weight_nucj_1);

                                if (state.score == newscore && abs(state.cai_score - cai_score) < epsilon){
                                    // Check if node i is connected to i-1 node with the current base at i-1
                                    // This check is needed to prevent S->T substitution errors
                                    if (i > 0) {
                                        bool i_back_connected = false;
                                        auto i_m1_nuc = GET_ACGU_NUC(sequence[i-1]);
                                        if (i_m1_nuc >= 1) {
                                            for (auto& i_left : dfa.left_edges[i_node]) {
                                                auto i_left_nuc = std::get<1>(i_left);
                                                if (i_left_nuc == i_m1_nuc) {
                                                    i_back_connected = true;
                                                    break;
                                                }
                                            }
                                            if (!i_back_connected)
                            ...
                            if(q_equ_j_1){
                                for (auto& i1_node_nuci : dfa.right_edges[i_node]){
                                    ...
                                    for (auto &p_node_nucp : p_list){
                                        ...
                                        auto f14_value = func14(i, j-1, p, q-1, nuci, nucp, nucq_1,
                                                                nucj_1, nuci, nucp, nucq_1, nucj_1);
                                        ScoreType newscore;
                                        if (p_state.score == util::value_min<ScoreType>())
                                            newscore = util::value_min<ScoreType>();
                                        else if (f14_value == util::value_max<ScoreType>())
                                        ...
                                        if (p == i+1){
                                            ...
                                            for (auto& i2_node_nuci1 : dfa.right_edges[i1_node]){
                                                ...
                                                for (auto& p_1_node_nucp_1 : dfa.left_edges[p_node]){
                                                    ...
                                                    auto f14_value = func14(i, j-1, p, q-1, nuci, nuci1, nucj_2,
                                                                            nucj_1, nucp_1, nucp, nucq_1, nucq);
                                                    if (p_state.score == util::value_min<ScoreType>())
                                                        newscore = util::value_min<ScoreType>();
                                                    else if (f14_value == util::value_max<ScoreType>())
                                                        newscore = util::value_min<ScoreType>();
            ...
            case Beam_type::BEAM_MULTI:
                ...
                for (auto& j_1_node_nucj_1 : dfa.left_edges[j_node]){
                    ...
                    for (IndexType p_node_ = 0; p_node_ < 2 * q; ++p_node_) {
                        ...
                        for (auto& i1_node_nuci : dfa.right_edges[i_node]){
                            ...
                            if(nuci != get<1>(i1_node_nuci)) continue;
            ...
            case Beam_type::BEAM_M2:
                ...
                for (IndexType m_node_nucpair_ = 0; m_node_nucpair_ < 16 * j; ++m_node_nucpair_){
