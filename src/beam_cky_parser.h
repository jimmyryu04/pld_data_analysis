...
struct BasepairingMask {
    int start5, end5;   // 5' side range, [start5, end5), 0-based
    int start3, end3;   // 3' side range, [start3, end3), 0-based
};

struct PLDOptions {
    struct BasepairingMask *basepairing_masks;
    int num_basepairing_masks;

    long random_rejection_threshold;
    long random_rejection_effective_threshold;
};
...
string get_nuc_from_dfa_cai(DFAType& dfa, const NodeType& start_node, const NodeType& end_node,
    ...
    if (t_index - s_index < 3) {
        if (s_index / 3 == t_index / 3) {
            ...
            if ((IndexType)temp_seq.length() != end_node.first - start_node.first) {
                assert(false);
            }
            return temp_seq;
        } else {
...
private:
    ...
    inline bool check_random_rejection(void) {
        return pld->random_rejection_threshold > 0 &&
               random() < pld->random_rejection_threshold;
    }

    void disable_random_rejection(void) {
        pld->random_rejection_effective_threshold = 0;
    }
    void enable_random_rejection(void) {
        pld->random_rejection_effective_threshold = pld->random_rejection_threshold;
    }

    void update_if_better(State_t &state, const ScoreType newscore, const double cai_score) {
        double score_diff = static_cast<double>(newscore) - static_cast<double>(state.score);
        double cai_diff = cai_score - state.cai_score;
        double total_diff = score_diff + cai_diff;

        if (total_diff > 0) {
            if (state.score != util::value_min<ScoreType>() && check_random_rejection())
                return;
